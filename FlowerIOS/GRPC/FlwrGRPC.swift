...
public class FlwrGRPC {
    ...
    private static let maxMessageLength: Int = 536_870_912
    private var bidirectionalStream: BidirectionalStreamingCall<Flwr_Proto_ClientMessage, Flwr_Proto_ServerMessage>?
    ...
    public init(serverHost: String, serverPort: Int, extendedInterceptor: InterceptorExtension? = nil) {
        ...
        eventLoopGroup = PlatformSupport
    ...
    public func startFlwrGRPC(client: Client, completion: @escaping () -> Void) {
        let grpcClient = Flwr_Proto_FlowerServiceNIOClient(channel: channel, interceptors: FlowerInterceptorsFactory(extendedInterceptor: extendedInterceptor))
        ...
        bidirectionalStream = grpcClient.join(callOptions: callOptions, handler: { sm in
            do {
    ...
    public func abortGRPCConnection(reasonDisconnect: ReasonDisconnect, completion: @escaping () -> Void) {
        ...
        _ = bidirectionalStream?.sendMessage(clientMessage)
